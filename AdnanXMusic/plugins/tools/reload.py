...
from AdnanXMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from AdnanXMusic.utils.decorators import ActualAdminCB, AdminActual, language
from AdnanXMusic.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Adnany.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Adnany.stop_stream_force(chat_id)
        except:
            pass
